...
import 'package:netsurf/models/ApiRequest.dart';
import 'package:netsurf/models/ApiResponse.dart';
...
Future<EventObject> checkData(String username, String pin) async {
  ApiRequest apiRequest = new ApiRequest();
  User user = new User(username: username, secret_pin: pin);

  apiRequest.operation = APIOperations.CHECK;
  apiRequest.user = user;
  ...
  try {
    ...
    if(response != null) {
      if(response.statusCode == APIResponseCode.SC_OK && response.body != null) {
        ...
        if(apiResponse.result == APIOperations.SUCCESS) {
          return new EventObject(
            id: EventConstants.CHECK_SUCCESSFUL,
            object: apiResponse.user
          );
        }
        else {
          return new EventObject(
            id: EventConstants.CHECK_ERR,
          );
        }
      }
      else if(response.statusCode == APIResponseCode.SC_ERR && response.body != null) {
